...
function modifier_mutation_kill_streak_power:GetTexture()
	return "custom/killstreak_power"
end
...
function modifier_mutation_kill_streak_power:OnCreated()
	...
	if IsServer() then
		self.particle = ParticleManager:CreateParticle("particles/powerrune_overhead/powerrune_overhead.vpcf", PATTACH_OVERHEAD_FOLLOW, self.parent)
		ParticleManager:SetParticleControlEnt(self.particle, 0, self.parent, PATTACH_OVERHEAD_FOLLOW, "attach_hitloc", self.parent:GetAbsOrigin() + Vector(0, 0, 128), true)
